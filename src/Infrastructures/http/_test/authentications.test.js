...
describe("/authentications endpoint", () => {
  ...
  describe("when POST /authentications", () => {
    it("should response 201 and new authentication", async () => {
      ...
      // Verify token was added to DB
      const tokensInDb = await AuthenticationsTableTestHelper.findToken(
        responseJson.data.refreshToken
      );
      expect(tokensInDb).toHaveLength(1);
      expect(tokensInDb[0].token).toEqual(responseJson.data.refreshToken);
    });
